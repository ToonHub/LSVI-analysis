---
title: 'Meetnet habitatkwaliteit: verkenning resultaten heidehabitats'
author: "Toon Westra"
...
```{r results ='hide', echo = FALSE, message = FALSE, cache = FALSE, purl = FALSE, warning=FALSE}
...
In dit document verkennen we de LSVI-indicatoren afgeleid uit de gegevens van het meetnet habitatkwaliteit. 

# Verkenning meetvariabelen

```{r Rinit,  warning = FALSE, message = FALSE, results = 'hide'}
...
* metadata: datum gegevensinzameling, coördinaten van plot en ligging t.o.v. SBZH. 
...
```{r specifySubtype,  cache = TRUE, warning = FALSE, message = FALSE, results = 'hide' }
...
\autoref{fig:ObservedHab} geeft het aantal opgemeten meetpunten per habitat(sub)type over de verschillende jaren. De figuur onderscheidt ook de meetpunten die volledig uit doelhabitat bestaan en de plots die gedeeltelijk uit doelhabitat bestaan.

```{r figHabtype,  fig.cap= "\\label{fig:ObservedHab}Aantal plots per habitat(sub)type"}
...
\autoref{tab:nMeasured} geeft een overzicht van het huidige aantal opgemeten meetpunten en het totale steekproefgrootte die we na 12 jaar willen berijken.
...
pandoc.table(overzicht, caption = "\\label{tab:nMeasured}Aantal opgemeten meetpunten in eerste drie jaar en totaal aantal gewenste meetpunten na meetcyclus van 12 jaar") 
...
In \autoref {tab:Londo} geven we de bedekkingen die gebruikt worden voor elke Londo-klasse. De transformatie naar bedekking (%) is nodig om de gezamelijke bedekking te kunnen berekenen van een groep van soorten (bv. bedekking door sleutelsoorten)
...
pandoc.table(cover, caption = "\\label{tab:Londo} Klassen van de Londo-schaal met bijhorende bedekking")
...
`r if (OWSoorten){"Voor volgende plots zitten er geen soortgegevens in de databank: "}`

`r if (OWSoorten){paste(plotsZonderSoorten, collapse = ", ")}`

`r if (!OWSoorten){"Voor alle plots werden er soortgegevens ingegeven in de databank."}`

`r if (OWBedekking){"Voor volgende plots ontbreekt de bedekking voor een of meerdere soorten: "}`
`r if (OWBedekking){paste(plotsBedekkingOntbreekt, collapse = ", ")}`
`r if (!OWBedekking){"Voor alle ingevoerde soorten werd de bedekking ingegeven"}`


### Dataverkenning

\autoref{figVeg} geeft een overzicht van het aantal getelde soorten per vegetatieplot en per vegetatielaag. 

```{r figuurVegOpname, warning = FALSE, message = FALSE, fig.cap = "\\label{figVeg} Aantal geobserveerde soorten in vegetatieplot"}
...
\autoref{figVeg2} toon de soorten die het vaakst voorkomen per habitat(subt)type en \autoref{figVeg3} geven de soorten met de hoogste gemiddelde bedekking.
...
```{r figuurVegOpname3, warning = FALSE, message = FALSE, fig.cap = "\\label{figVeg2} Tien meest voorkomende soorten per habitat(sub)type"}    
...
```{r figuurVegOpname4, warning = FALSE, message = FALSE, fig.cap = "\\label{figVeg3} Tien meest bedekkende soorten per habitat(sub)type"}
...
Ontbrekende waarden voor bedekking komen overeen met een bedekking van 0%. Dit moet nog aangepast worden in de datbank.
...
`r if (OWStructuurdata){"Voor volgende plots zitten er geen structuurgegevens in de databank: "}`
`r if (OWStructuurdata){plotsZonderStructuurdata}`
`r if (!OWStructuurdata){"Voor alle plots werden er structuurgegevens ingegeven in de databank."}`

### Dataverkenning

Hieronder geven we een overzicht van de opgemeten bedekkingen voor de meetvariabelen m.b.t. de verstoringsindicatoren (\autoref{fig:verstoring}) en de habitatstructuurindicatoren (\autoref{fig:structuur1} en \autoref{fig:structuur2}).

```{r figuurStructuurPlot, warning = FALSE, message = FALSE, fig.cap = "\\label{fig:verstoring}  Meetvaribelen m.b.t. de LSVI-verstoringsindicatoren"}
...
```{r figuurStructuurPlot2, warning = FALSE, message = FALSE, fig.cap = "\\label{fig:structuur1}  Meetvaribelen m.b.t. de LSVI-habitatstructuurindicatoren"}
...
```{r figuurStructuurPlot3, warning = FALSE, message = FALSE, fig.cap = "\\label{fig:structuur2}  Totale bedekking van Struikhei en bedekking van de verschillende fase van Struikhei "}
...
```{r tabelIndicatoren}
#kable(overzicht_indicatoren, align = "c", row.names = FALSE)

#as_tibble(indicatoren[,c("HabCode","IDPlots","AnalyseVariabele", "Eenheid", "Waarde", "Beoordeling")])
```
...
In \autoref{bijlage-1-overzicht-analysevariabelen-van-de-meetpunten} geven we voor elk habitattype de beoordelingstabel voor LSVI-versie2 en LSVI-versie3. Drempelwaarden uitgedrukt in abundantieklassen werden vertaald naar percentages bedekking (NA TE KIJKEN!) zodat de indicatoren beoordeeld kunnen worden aan de hand van de ingezamelde gegevens. De waarden en beoordeling van de analysevariabelen voor de verschillende meetpunten worden ook weergegeven in \autoref{bijlage-1-overzicht-analysevariabelen-van-de-meetpunten}.  


```{r verkenningIndPlots2, warning = FALSE, message = FALSE, fig.height=7}
...
\autoref{figAV} toont voor elke analysevariabele van LSVI-versie3 het gemiddelde en het bijhorende 95% betrouwbaarheidsinterval. \autoref{figAV_LSVI2} geeft dit weer voor LSVI-versie2.Gezien de meeste variabelen niet normaal verdeeld zijn, worden de betrouwbaarheidsintervallen berekend via bootstrapping. Zo vermijden we bijvoorbeeld dat de grenzen van de betrouwbaarheidsintervallenvoor bedekkingen kleiner dan 0 of groter dan 100 zijn. Ter informatie tonen we in de figuur ook de drempelwaarden, hoewel deze drempelwaarden niet bedoeld zijn om een gemiddelde van een indicator aan te toetsen.

In de analyse hebben we nog geen rekening gehouden met het feit dat sommige meetpunten slechts gedeeltelijk uit doelhabitat bstaan. Hiervoor zouden we moeten gebruik maken van gewichten per meetpunt: to do. 


```{r verkenningAV_LSVI3, warning = FALSE, message = FALSE, fig.height=8, fig.cap= "\\label{figAV} Gemiddelde waarde en 95% betrouwbaarheidsinterval van de analysevariabelen van LSVI-versie3"}
indicatoren %>%
...
# 
# ggplot(indicatoren, aes(x = AnalyseVariabele, fill = BeoordelingTekst)) + geom_bar(width = 0.5) + facet_grid(Criterium ~ HabCode , scales = "free", space = "free_y") + labs(x = "Analysevariabele" ,fill = "Beoordeling",  y = "Aantal meetpunten") + scale_fill_manual(values = colors_beoordeling) + geom_hline(aes(yintercept = Grens), colour = "black", size = 0.5, linetype = "dashed", alpha = 0.5)  +
#   theme(strip.text.y = element_text(angle = 0), legend.position = "bottom")  + coord_flip() 
```

```{r verkenningAV_LSVI2, warning = FALSE, message = FALSE, fig.height=8, fig.cap= "\\label{figAV_LSVI2} Gemiddelde waarde en 95% betrouwbaarheidsinterval van de analysevariabelen van LSVI-versie2"}
indicatoren %>%
...
\autoref{fig:indicatoren} en \autoref{fig:indicatoren_LSVI2} tonen per habitattype en per indicator het aantal gunstige en ongunstige meetpunten voor respectievelijk LSVI-versie3 en LSVI-versie2. De stippelijn komt overeen met 25% van de meetpunten. De 25%-grens werd in de habitatrapportage van 2013 gebruikt om de verschillende indicatoren te beoordelen op schaal Vlaanderen: een indicator voor een habitattype wordt globaal gezien als gunstig beschouwd als minder dan 25% van het habitat lokaal ongunstig is. 

```{r aggergatiePerIndicator5, fig.cap= "\\label{fig:indicatoren} Aantal ongunstige en ongunstige meetpunten per indicator van LSVI-versie3; de stippellijn geeft de 25%-grens weer"}
...
```{r aggergatiePerIndicator_LSVI2, fig.cap= "\\label{fig:indicatoren_LSVI2} Aantal ongunstige en ongunstige meetpunten per indicator van LSVI-versie2; de stippellijn geeft de 25%-grens weer"}
...
Opvallend is het verschil tussen beide versies van de LSVI voor de indicator sleutelsoorten, vooral voor habitatsubtype 2330_bu en habitattype 4030. Dit heeft enerzijds te maken met de drempelwaarden die verschillen tussen beide versies, maar ook met de lijst van sleutelsoorten die verschillen \autoref{fig:sleutelsoorten}.Voor 2330_bu zien we bijvoorbeeld dat in een groot aantal meetpunten Zandstruisgras werd geobserveerd, wat een sleutelsoort is in LSVI-versie2 maar niet in LSVI-versie3. 

```{r sleutelsoorten, fig.cap = "\\label{fig:sleutelsoorten}Overzicht van de geobserveerde sleutelsoorten per habitat(sub)type en voor de verschillende versies van de LSVI"}
...
Voor de beoordeling van de indicatoren op schaal Vlaanderen testen we of het aandeel meetpunten dat ongunstig scoort significant verschilt van 25%. Hiervoor gebruiken we een tweezijdige biomiale test. Indien het aandeel meetpunten dat ongunstig scoort niet significant verschilt van 25%, kunnen we geen uitspraak doen (Beoordeling = "Unknown"). 

\autoref{fig:indicatoren2} en \autoref{fig:indicatoren2_LSVI2} tonen per indicator en per habitat(sub)type het geschatte aandeel dat ongunstig scoort en de bijhorende 95%-betrouwbaarheidsintervallen respectievelijk voor voor LSVI-versie3 en voor LSVI-versie2.  Voor heel wat indicatoren is er nog geen uitspraak mogelijk. Dit heeft uiteraard te maken met de (voorlopig nog) beperkte steekproefgrootte. Ter herinnering: de totale steekpreofgrootte per habitattype is 170 en hiermeee kunnen we een absoluut verschil van ongeveer 10% (t.o.v. de  25%-grens) detecteren met een onderscheidend vermogen van 80% en een significantieniveau van 5%.
...
```{r aggergatiePerIndicator4, fig.cap= "\\label{fig:indicatoren2} Geschatte aandeel van habitat(sub)type dat ongunstig scoort voor de verschillende indicatoren van LSVI-versie3, de bijhoreden betrouwbaarheidsintervallen en de beoordeling op Vlaamse schaal bij significantieniveau van 0.05"}
...
```{r aggergatiePerIndicator2_LSVI2, fig.cap= "\\label{fig:indicatoren2_LSVI2} Geschatte aandeel van habitat(sub)type dat ongunstig scoort voor de verschillende indicatoren van LSVI-versie2, de bijhoreden betrouwbaarheidsintervallen en de beoordeling op Vlaamse schaal bij significantieniveau van 0.05"}
...
Het aandeel ongunstige meetpunten voor indicator 'invasieve exoten' van habiattype 2330_bu en 4030 (respectievelijk 10% en 11%) is net niet significant verschillend van 25%, met p-waarden die iets hoger liggen dan 0.05. In dergelijke gevallen kunnen we met voldoende zekerheid oordelen dat de indicatoren gunstig zijn op Vlaamse schaal. Voor 4030 is deze indicator echter enkel opgenomen in LSVI-versie3.   

Wanneer we 0.10 als significantieniveau nemen dan zijn er uiteraard minder indicatoren waarvoor er geen beoordeling mogelijk is (\autoref{fig:indicatoren3} en \autoref{fig:indicatoren3_LSVI2}). Dit gaat samen met een hogere type I-fout: de kans dat we een verschil detecteren op basis van het meetnet wanneer er in de werkelijkheid geen verschil is bedraagt 10% i.p.v. 5%. Dit is een afweging die gemaakt moet worden bij de uiteindelijke rapportage.

```{r aggergatiePerIndicator3, fig.cap= "\\label{fig:indicatoren3} Geschatte aandeel van habitat(sub)type dat ongunstig scoort voor de verschillende LSVI-indicatoren, de bijhoreden betrouwbaarheidsintervallen en de beoordeling op Vlaamse schaal bij significantieniveau van 0.10"}
...
```{r aggergatiePerIndicator3_LSVI2, fig.cap= "\\label{fig:indicatoren3_LSVI2} Geschatte aandeel van habitat(sub)type dat ongunstig scoort voor de verschillende LSVI-indicatoren, de bijhoreden betrouwbaarheidsintervallen en de beoordeling op Vlaamse schaal bij significantieniveau van 0.10"}
...
vergelijking %>%
  ...
  summarise(`Meetnet favourable` = sum(`Meetnet habitatkwaliteit` == "Favourable"),
            `Meetnet unknown` = sum(`Meetnet habitatkwaliteit` == "Unknown"),
            `Meetnet unfavourable` = sum(`Meetnet habitatkwaliteit` == "Unfavourable")) %>%
  pandoc.table(caption = "\\label{tab:verRapportage} Vergelijking beoordeling indicatoren", split.cell = 15)
...
Om de beoordeling minder afhankelijk te maken van de schaal, zouden we ook de gemiddelde waarde van een analysevariabele/indicator kunnen in rekening nemen bij de beoordeling.  




# Bijlage 1 Overzicht analysevariabelen van de meetpunten

## Habitattype 2310

```{r verkenningAV_2310, warning = FALSE, message = FALSE, results = 'asis'}

pandoc.table(overzicht_indicatoren %>%
               filter(HabCode == "2310", VersieLSVI == "versie3") %>%
               select(-HabCode, -VersieLSVI),
             split.cells = c(20, 10, 13, 5,5),
             caption = "\\label{tab:LSVIhab2310}Overzicht analysevariabelen voor LSVI-versie3 voor habitattype 2310")

```


```{r verkenningAV_2310_LSVI2, warning = FALSE, message = FALSE, results = 'asis'}

pandoc.table(overzicht_indicatoren %>%
               filter(HabCode == "2310", VersieLSVI == "versie2") %>%
               select(-HabCode, -VersieLSVI),
             split.cells = c(20, 10, 13, 5,5),
             caption = "\\label{tab:LSVIhab2310}Overzicht analysevariabelen voor LSVI-versie2 voor habitattype 2310")
...
```{r figuurAV_2310, warning = FALSE, message = FALSE, fig.height=6.5, fig.cap= "\\label{fig:fig2310} Overzicht analysevariabelen van LSVI-versie3 voor de meetpunten van habitattype 2310"}
...
```{r figuurAV_2310_LSVI2, warning = FALSE, message = FALSE, fig.height=6.5, fig.cap= "\\label{fig:fig2310_LSVI2} Overzicht analysevariabelen van LSVI-versie2 voor de meetpunten van habitattype 2310"}
...
```{r verkenningAV_2330_bu, warning = FALSE, message = FALSE, results = 'asis'}

pandoc.table(overzicht_indicatoren %>%
               filter(HabCode == "2330_bu", VersieLSVI == "versie3") %>%
               select(-HabCode, -VersieLSVI),
             split.cells = c(20, 10, 13, 5,5),
             caption = "\\label{tab:LSVIhab2330_bu}Overzicht analysevariabelen voor LSVI-versie3 voor habitattype 2330_bu")

```


```{r verkenningAV_2330_bu_LSVI2, warning = FALSE, message = FALSE, results = 'asis'}

pandoc.table(overzicht_indicatoren %>%
               filter(HabCode == "2330_bu", VersieLSVI == "versie2") %>%
               select(-HabCode, -VersieLSVI),
             split.cells = c(20, 10, 13, 5,5),
             caption = "\\label{tab:LSVIhab2330_bu}Overzicht analysevariabelen voor LSVI-versie2 voor habitattype 2330_bu")

```


```{r figuurAV_2330_bu, warning = FALSE, message = FALSE, fig.height=6.5, fig.cap= "\\label{fig:fig2330_bu} Overzicht analysevariabelen van LSVI-versie3 voor de meetpunten van habitattype 2330_bu"}
...
```{r figuurAV_2330_bu_LSVI2, warning = FALSE, message = FALSE, fig.height=6.5, fig.cap= "\\label{fig:fig2330_bu_LSVI2} Overzicht analysevariabelen van LSVI-versie2 voor de meetpunten van habitattype 2330_bu"}
...
```{r verkenningAV_4010, warning = FALSE, message = FALSE, results = 'asis'}

pandoc.table(overzicht_indicatoren %>%
               filter(HabCode == "4010", VersieLSVI == "versie3") %>%
               select(-HabCode, -VersieLSVI),
             split.cells = c(20, 10, 13, 5,5),
             caption = "\\label{tab:LSVIhab4010}Overzicht analysevariabelen voor LSVI-versie3 voor habitattype 4010")

```


```{r verkenningAV_4010_LSVI2, warning = FALSE, message = FALSE, results = 'asis'}

pandoc.table(overzicht_indicatoren %>%
               filter(HabCode == "4010", VersieLSVI == "versie2") %>%
               select(-HabCode, -VersieLSVI),
             split.cells = c(20, 10, 13, 5,5),
             caption = "\\label{tab:LSVIhab4010}Overzicht analysevariabelen voor LSVI-versie2 voor habitattype 4010")

```


```{r figuurAV_4010, warning = FALSE, message = FALSE, fig.height=6.5, fig.cap= "\\label{fig:fig4010} Overzicht analysevariabelen van LSVI-versie3 voor de meetpunten van habitattype 4010"}
...
```{r figuurAV_4010_LSVI2, warning = FALSE, message = FALSE, fig.height=6.5, fig.cap= "\\label{fig:fig4010_LSVI2} Overzicht analysevariabelen van LSVI-versie2 voor de meetpunten van habitattype 4010"}
...
```{r verkenningAV_4030, warning = FALSE, message = FALSE, results = 'asis'}

pandoc.table(overzicht_indicatoren %>%
               filter(HabCode == "4030", VersieLSVI == "versie3") %>%
               select(-HabCode, -VersieLSVI),
             split.cells = c(20, 10, 13, 5,5),
             caption = "\\label{tab:LSVIhab4030}Overzicht analysevariabelen voor LSVI-versie3 voor habitattype 4030")

```


```{r verkenningAV_4030_LSVI2, warning = FALSE, message = FALSE, results = 'asis'}

pandoc.table(overzicht_indicatoren %>%
               filter(HabCode == "4030", VersieLSVI == "versie2") %>%
               select(-HabCode, -VersieLSVI),
             split.cells = c(20, 10, 13, 5,5),
             caption = "\\label{tab:LSVIhab4030}Overzicht analysevariabelen voor LSVI-versie2 voor habitattype 4030")

```


```{r figuurAV_4030, warning = FALSE, message = FALSE, fig.height=6.5, fig.cap= "\\label{fig:fig4030} Overzicht analysevariabelen van LSVI-versie3 voor de meetpunten van habitattype 4030"}
...
```{r figuurAV_4030_LSVI2, warning = FALSE, message = FALSE, fig.height=6.5, fig.cap= "\\label{fig:fig4030_LSVI2} Overzicht analysevariabelen van LSVI-versie2 voor de meetpunten van habitattype 4030"}
